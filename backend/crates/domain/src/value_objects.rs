...
impl Email {
    pub fn new(email: String) -> Result<Self> {
        ...
        if !email_regex.is_match(&email) {
            return Err(DomainError::ValidationFailed("Invalid email format".to_string()));
        }
...
impl TenantId {
    pub fn new(id: String) -> Result<Self> {
        if id.is_empty() || id.len() > 50 {
            return Err(DomainError::ValidationFailed("Tenant ID must be 1-50 characters".to_string()));
        }
        ...
        if !id_regex.is_match(&id) {
            return Err(DomainError::ValidationFailed("Tenant ID can only contain alphanumeric characters, underscores, and hyphens".to_string()));
        }
...
impl RfqId {
    pub fn new(id: String) -> Result<Self> {
        if id.is_empty() || id.len() > 50 {
            return Err(DomainError::ValidationFailed("RFQ ID must be 1-50 characters".to_string()));
        }
...
impl ManufacturerId {
    pub fn new(id: String) -> Result<Self> {
        if id.is_empty() || id.len() > 50 {
            return Err(DomainError::ValidationFailed("Manufacturer ID must be 1-50 characters".to_string()));
        }
        
        let id_regex = Regex::new(r"^mfg_[a-zA-Z0-9_-]+$")
            .map_err(|_| DomainError::Internal("Failed to compile manufacturer ID regex".to_string()))?;
...
impl S3Key {
    pub fn new(key: String) -> Result<Self> {
        if key.is_empty() || key.len() > 1024 {
            return Err(DomainError::ValidationFailed("S3 key must be 1-1024 characters".to_string()));
        }
        
        // Basic validation - no leading slash, no double slashes
        if key.starts_with('/') || key.contains("//") {
            return Err(DomainError::ValidationFailed("Invalid S3 key format".to_string()));
        }
...
impl FileSize {
    ...
    pub fn new(size_bytes: u64) -> Result<Self> {
        if size_bytes == 0 {
            return Err(DomainError::ValidationFailed("File size cannot be zero".to_string()));
        }
...
impl MessageBody {
    ...
    pub fn new(body: String) -> Result<Self> {
        if body.is_empty() {
            return Err(DomainError::ValidationFailed("Message body cannot be empty".to_string()));
        }
        ...
        if body.contains('<') && body.contains('>') {
            return Err(DomainError::ValidationFailed("HTML tags are not allowed in message body".to_string()));
        }
